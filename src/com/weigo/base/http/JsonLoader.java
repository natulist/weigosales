...
public class JsonLoader implements AbortableInternetLoader {
	...
	public boolean load() {
		...
		return mListLoader.load(paramsMap, entityMap, mProvider.supportPost(), mProvider.getHttpsCer());
	}
		...
		public void onLoaded(int err, HttpEntity entity) {
			...
			if (err == NetworkError.SUCCESS && entity != null) {
				...
				if (obj == null) {
					...
					//FIXME
//					if (err == LoginError.LE_TOKEN_VALID || err == ErrorCode.EC_TOKEN_INVALID) {
//						NotificationCenter.defaultCenter().publish("TokenValid", new AsyncDataEvent());
//					}
				}
