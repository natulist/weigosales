...
public class HttpTask extends Thread {
	...
	public HttpTask(HttpRequestBase request) {
		mRequest = request;
		mClient = createClient(request);
	}
	
	public HttpTask(HttpRequestBase request, String cer) {
		mRequest = request;
		mClient = createClient(request);
		mCer = cer;
	}
	...
	private boolean isHttps(HttpRequestBase request) {
		...
		return uri.getScheme().equalsIgnoreCase("https") && (mCer!=null);
	}
	
	private HttpClient createClient(HttpRequestBase request) {
